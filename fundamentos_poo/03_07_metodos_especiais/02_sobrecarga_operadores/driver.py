# Neste desafio, você implementará uma classe Money que representa valores monetários com sobrecarga robusta de operadores. 
# Sua implementação será testada minuciosamente contra uma suíte de testes abrangente.

# money.py - Este é o único arquivo que você precisa editar. Ele contém a definição da classe com comentários TODO 
# que guiam sua implementação.
# driver.py - Contém cenários de teste extensivos que validam sua implementação (não modifique).
# Implemente a classe Money com as seguintes funcionalidades:

# Construtor que recebe amount (float) e currency (string)
# Soma (+) de objetos Money com a mesma moeda
# Multiplicação (*) por um número para escalar o valor
# Comparação de igualdade (==) entre objetos Money
# Representação em string no formato “X.XX CUR”
